...
class Ftp extends Locale {
    ...
    public function __construct($config){
        ...
        /* 登录FTP服务器 */
        if(!$this->login()){
            SharinException::throwing($this->error);
        }
    }
...
//        extract($this->config);
        $this->link = ftp_connect($this->host, $this->port, $this->timeout);
        if($this->link) {
            if (ftp_login($this->link, $this->username, $this->password)) {
                return true;
            } else {
                $this->error = "无法登录到FTP服务器：username - {$this->username}";
            }
        } else {
            $this->error = "无法连接到FTP服务器：{$this->host}";
        }
        return false;
    }
