<?php
/**
 * Created by PhpStorm.
 * User: lnzhv
 * Date: 7/25/16
 * Time: 2:45 PM
 */

namespace App\Test\Demo\Controller;


use Sharin\Core\Controller;

class Component extends Controller {

    public function boxView(){
        $this->display();
    }
    public function buttons(){
        $this->display();
    }

    public function c3chart(){
        $this->display();
    }

    public function calendar(){
        $this->display();
    }

    public function chartjs(){
        $this->display();
    }

    /**
     * 拖拽上传
     */
    public function dropzone(){
        $this->display();
    }
    public function editors(){
        $this->display();
    }
    public function flotChart(){
        $this->display();
    }
    public function fontawesome(){
        $this->display();
    }

    public function gallery(){
        $this->display();
    }

    public function general(){
        $this->display();
    }
    public function map(){
        $this->display();
    }

    public function grids(){
        $this->display();
    }

    public function horizontal_menu(){
        $this->display();
    }
    public function index(){
        $this->display();
    }
    public function inline_editors(){
        $this->display();
    }

    public function invoice(){
        $this->display();
    }

    public function invoice_print(){
        $this->display();
    }

    public function leftmenu_collapsed_view(){
        $this->display();
    }


    public function morris(){
        $this->display();
    }
    public function nestable(){
        $this->display();
    }
    public function panels(){
        $this->display();
    }
    public function pickers(){
        $this->display();
    }
    public function pricing_table(){
        $this->display();
    }

    public function profile(){
        $this->display();
    }
    public function register(){
        $this->display();
    }

    public function slider(){
        $this->display();
    }
    public function tabs_accordions(){
        $this->display();
    }
    public function vector_map(){
        $this->display();
    }


    public function timeline(){
        $this->display();
    }



    public function tree_view(){
        $this->display();
    }

    public function typography(){
        $this->display();
    }




}