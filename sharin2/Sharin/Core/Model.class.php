...
use Sharin\SharinException as Exception;
use Sharin\Utils;
...
//        static $c = 0;
        if(null === $tablename){
            ...
            //设置更新字段
//            \Sharin\dumpout($this->_options,$this->_inputs);
...
//            \Sharin\dumpout($sql,$inputs,$this->query($sql,$inputs),$this->error);
...
//        extract($components,EXTR_OVERWRITE);
        ...
        if(!empty($components['join'])){
            if(is_array($components['join'])){
                ...
                Exception::throwing('Wrong join for select!',$components['join']);//不为空非法
            }
        }
