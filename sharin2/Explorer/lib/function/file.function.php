...
namespace {
     ...
     * 文件下载到服务器		file_download_this($from, $file)
...
//        }
    ...
    * 修改文件、文件夹权限
    ...
    function size_format($bytes, $precision = 2){
        return Storage::formatSize($bytes,$precision);
    }
    ...
    /**
     * 远程文件下载到服务器
     * @param string $url  支持fopen的打开都可以；支持本地、url
     *                      如果以 "scheme://..."开头则自动搜索指定的协议处理器进行处理
     * @param string $file 保存文件名词
     * @return bool
     */
    function file_download_this($url, $file=''){
        return Network::download($url, $file);
    }
