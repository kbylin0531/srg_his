...
class Verify extends \Soya{
    ...
    public function __construct(){
        parent::__construct(SINGLE_INSTANCE);
        $this->config = self::getConfig();//兼容之前的代码
    }
    ...
    public function check($code, $id = '') {
        ...
        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->config['reset'] && $session->set($key, null);
            return true;
        }
        ...
        if($this->config['useZh']){ // 中文验证码
            for ($i = 0; $i<$this->config['length']; $i++) {
                ...
                imagettftext(
                    ...
                    $this->config['fontSize'] + mt_rand(10, 20),
                    $this->_color,
                    $this->config['fontttf'],
                    $code[$i]);
            }
            ...
            for ($i = 0; $i<$this->config['length']; $i++) {
                ...
                imagettftext($this->_image, $this->config['fontSize'], mt_rand(-40, 40), $codeNX, $this->config['fontSize']*1.6, $this->_color, $this->config['fontttf'], $code[$i]);
            }
        }
