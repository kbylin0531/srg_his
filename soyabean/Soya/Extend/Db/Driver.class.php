...
//                    trace($e->getMessage(),'','ERR');
                    ...
                    Exception::throwing($e->getMessage());
                }
            }
        }
...
//        trace($this->error,'','ERR');
        if($this->config['debug']) {// 开启数据库调试模式
            return Exception::throwing($this->error);
//            E($this->error);
        }else{
            return $this->error;
        }
    ...
    protected function parseValue($value) {
        if(is_string($value)) {
            $value =  strpos($value,':') === 0 && in_array($value,array_keys($this->bind))? $this->escapeString($value) :
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                if(preg_match('/^(eq|neq|gt|egt|lt|elt)$/',$exp)) { // 比较运算
                    ...
                    Exception::throwing("_EXPRESS_ERROR_:{$val[0]}");
...
//            .$this->parseDuplicate();
     ...
     * @param array $options 参数表达式
     * @_param boolean $replace 是否replace
     * @return false | integer
     */
    public function insertAll($dataSet,$options=array()) {
        $values  =  array();
        $this->model  =   $options['model'];
...
//        }
        ...
        return $this->connect($db_config,$r,$r == $m ? false : array(
            'username'  =>  isset($_config['username'][$m])?$_config['username'][$m]:$_config['username'][0],
            'password'  =>  isset($_config['password'][$m])?$_config['password'][$m]:$_config['password'][0],
            'hostname'  =>  isset($_config['hostname'][$m])?$_config['hostname'][$m]:$_config['hostname'][0],
            'hostport'  =>  isset($_config['hostport'][$m])?$_config['hostport'][$m]:$_config['hostport'][0],
