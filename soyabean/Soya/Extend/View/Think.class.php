...
//----------------------------------------- 修改自ThinkPHP --------------------------------------------------------------------------//
    ...
    public function fetch($templateFile,$templateVar,$prefix='') {
        ...
        \Soya::loadTemplate($cachefile,$this->tVar);
    }
...
//                    \Soya\dumpout($templateFile,$tmplCacheFile,$storage->mtime($templateFile),$storage->mtime($tmplCacheFile));
    ...
    protected function parsePhp($content) {
        ...
        if($this->_config('DENY_PHP') && false !== strpos($content,'<?php')) {
            Exception::throwing('denyed using raw php in template!');
        }
        return $content;
    }
...
//        \Soya\dumpout($begin,$end,$matches);
    ...
    private function parseXmlAttrs($attrs) {
        ...
        if(!$xml)  return Exception::throwing('_XML_TAG_ERROR_:'.$attrs);
...
//                    elseif(C('SESSION_PREFIX')){$parseStr = '$_SESSION[\''.C('SESSION_PREFIX').'\'][\''.$vars[2].'\']';}
    ...
    public function parseXmlAttr($attr,$tag) {
        ...
        $xml    =   simplexml_load_string($xml);
        $xml or Exception::throwing('_XML_TAG_ERROR_:'.$attr);
        $xml    =   (array)($xml->tag->attributes());
            ...
            if($array) {
                ...
                foreach($attrs as $name) {
                    if( isset($array[$name])) {
                        ...
                        return Exception::throwing('_PARAM_ERROR_:'.$name);
                    }
                }
                return $array;
            }
