...
class Logger extends \Soya{
    ...
    protected static function fetchLogUID($level=self::LOG_LEVEL_DEBUG,$datetime=null){
        if(isset($datetime)){
            ...
            $datetime = Datetime::getDate(false);
            $rate = self::getConfig('LOG_RATE');
            $rate or $rate = self::LOGRATE_DAY;
            switch($rate){
                case self::LOGRATE_DAY:
                    $path = PATH_RUNTIME."Log/{$level}/{$datetime[1]}.log";
                    break;
                case self::LOGRATE_HOUR:
                    $path = PATH_RUNTIME."Log/{$level}/{$datetime[1]}/{$datetime[2]}.log";
                    break;
                default:
     ...
     */
    public function write($content,$level=self::LOG_LEVEL_DEBUG){
        return $this->_driver->write(self::fetchLogUID($level),$content);
    }
     ...
     */
    public function read($datetime, $level=self::LOG_LEVEL_DEBUG){
        return $this->_driver->read(self::fetchLogUID($level,$datetime));
    }
