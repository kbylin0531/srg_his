/*!
 *  Echo v1.4.0
 *  Lazy-loading with data-* attributes, offsets and delay options
...
window.Echo = (function (window, document) {
    ...
    /**
     * 判断元素是否在视野内部,包括offset在内
     * @param el 元素DOM
     * @returns {boolean}
     * @private
     */
    var inStage = function (el) {
        var coords = el.getBoundingClientRect();//获取元素位置
        return ((coords.top >= 0 && coords.left >= 0 && coords.top) <=
        (window.innerHeight || document.documentElement.clientHeight) + parseInt(convention['offset']));
    };
     ...
     * @private
     */
    var pollImages = function () {
        for (var i = imgs.length; i--;) {
            if (inStage(imgs[i])) {
                imgs[i].src = imgs[i].getAttribute(convention['attr']);
                imgs.splice(i, 1);//从i处删除已经完成图片加载的元素
            }
        }
    };

    /**
     * 如果拉过头,中间的图片不在stage,可以避免这些图片的加载
     * @private
     */
    var _delay = function () {
        clearTimeout(timer);//清除上一个定时器
        timer = setTimeout(pollImages, delay);
    };

    var init = function (obj) {
        if(obj) for (var x in obj) {
            if (!obj.hasOwnProperty(x) || !convention.hasOwnProperty(x)) continue;
            convention[x] = obj[x];
        }

        var nodes = document.querySelectorAll('[' + convention['attr'] + ']');//IE8开始支持querySelectorAll
        for (var i = 0; i < nodes.length; i++) {
            nodes[i].src = convention['blank'];//将src设置为空白背景
            nodes.style.backgroundRepeat = 'no';
            nodes.style.backgroundPosition = '40%';
            nodes.style.backgroundImage = 'url('+convention['loading']+')';
            imgs.push(nodes[i]);
        }
         ...
         *target： 文档节点、document、window 或 XMLHttpRequest。
         ...
         useCapture ：是否使用捕捉，一般用 false (捕捉之后无法向下传递)。例如：document.getElementById("testText").addEventListener("keydown", function (event) { alert(event.keyCode); }, false);
        ...
        if (document.addEventListener) {
            window.addEventListener('scroll', _delay, false);//IE9开始支持
        } else {
            window.attachEvent('onscroll', _delay);
        }
    };
