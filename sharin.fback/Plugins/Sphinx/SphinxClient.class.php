...
namespace Sharin\Vendor\Sphinx {
...
/// known searchd status codes
    defined('SEARCHD_OK') or define ( "SEARCHD_OK",				0 );
    defined('SEARCHD_ERROR') or define  ( "SEARCHD_ERROR",			1 );
    defined('SEARCHD_RETRY') or define  ( "SEARCHD_RETRY",			2 );
    defined('SEARCHD_WARNING') or define  ( "SEARCHD_WARNING",			3 );

/// known match modes
    defined('SPH_MATCH_ALL') or define ( "SPH_MATCH_ALL",			0 );
    defined('SPH_MATCH_ANY') or define ( "SPH_MATCH_ANY",			1 );
    defined('SPH_MATCH_PHRASE') or define ( "SPH_MATCH_PHRASE",		2 );
    defined('SPH_MATCH_BOOLEAN') or define ( "SPH_MATCH_BOOLEAN",		3 );
    defined('SPH_MATCH_EXTENDED') or define ( "SPH_MATCH_EXTENDED",		4 );
    defined('SPH_MATCH_FULLSCAN') or define ( "SPH_MATCH_FULLSCAN",		5 );
    defined('SPH_MATCH_EXTENDED2') or define ( "SPH_MATCH_EXTENDED2",		6 );	// extended engine V2 (TEMPORARY, WILL BE REMOVED)

/// known ranking modes (ext2 only)
    defined('SPH_RANK_PROXIMITY_BM25') or define ( "SPH_RANK_PROXIMITY_BM25",	0 );	///< default mode, phrase proximity major factor and BM25 minor one
    defined('SPH_RANK_BM25') or define ( "SPH_RANK_BM25",			1 );	///< statistical mode, BM25 ranking only (faster but worse quality)
    defined('SPH_RANK_NONE') or define ( "SPH_RANK_NONE",			2 );	///< no ranking, all matches get a weight of 1
    defined('SPH_RANK_WORDCOUNT') or define ( "SPH_RANK_WORDCOUNT",		3 );	///< simple word-count weighting, rank is a weighted sum of per-field keyword occurence counts
    defined('SPH_RANK_PROXIMITY') or define ( "SPH_RANK_PROXIMITY",		4 );
    defined('SPH_RANK_MATCHANY') or define ( "SPH_RANK_MATCHANY",		5 );
    defined('SPH_RANK_FIELDMASK') or define ( "SPH_RANK_FIELDMASK",		6 );
    defined('SPH_RANK_SPH04') or define ( "SPH_RANK_SPH04",			7 );
    defined('SPH_RANK_EXPR') or define ( "SPH_RANK_EXPR",			8 );
    defined('SPH_RANK_TOTAL') or define ( "SPH_RANK_TOTAL",			9 );

/// known sort modes
    defined('SPH_SORT_RELEVANCE') or define ( "SPH_SORT_RELEVANCE",		0 );
    defined('SPH_SORT_ATTR_DESC') or define ( "SPH_SORT_ATTR_DESC",		1 );
    defined('SPH_SORT_ATTR_ASC') or define ( "SPH_SORT_ATTR_ASC",		2 );
    defined('SPH_SORT_TIME_SEGMENTS') or define ( "SPH_SORT_TIME_SEGMENTS", 	3 );
    defined('SPH_SORT_EXTENDED') or define ( "SPH_SORT_EXTENDED", 		4 );
    defined('SPH_SORT_EXPR') or define ( "SPH_SORT_EXPR", 			5 );

/// known filter types
    defined('SPH_FILTER_VALUES') or define ( "SPH_FILTER_VALUES",		0 );
    defined('SPH_FILTER_RANGE') or define ( "SPH_FILTER_RANGE",		1 );
    defined('SPH_FILTER_FLOATRANGE') or define ( "SPH_FILTER_FLOATRANGE",	2 );
    defined('SPH_FILTER_STRING') or define ( "SPH_FILTER_STRING",	3 );

/// known attribute types
    defined('SPH_ATTR_INTEGER') or define ( "SPH_ATTR_INTEGER",		1 );
    defined('SPH_ATTR_TIMESTAMP') or define ( "SPH_ATTR_TIMESTAMP",		2 );
    defined('SPH_ATTR_ORDINAL') or define ( "SPH_ATTR_ORDINAL",		3 );
    defined('SPH_ATTR_BOOL') or define ( "SPH_ATTR_BOOL",			4 );
    defined('SPH_ATTR_FLOAT') or define ( "SPH_ATTR_FLOAT",			5 );
    defined('SPH_ATTR_BIGINT') or define ( "SPH_ATTR_BIGINT",			6 );
    defined('SPH_ATTR_STRING') or define ( "SPH_ATTR_STRING",			7 );
    defined('SPH_ATTR_FACTORS') or define ( "SPH_ATTR_FACTORS",			1001 );
    defined('SPH_ATTR_MULTI') or define ( "SPH_ATTR_MULTI",			0x40000001 );
    defined('SPH_ATTR_MULTI64') or define ( "SPH_ATTR_MULTI64",			0x40000002 );

/// known grouping functions
    defined('SPH_GROUPBY_DAY') or define ( "SPH_GROUPBY_DAY",			0 );
    defined('SPH_GROUPBY_WEEK') or define ( "SPH_GROUPBY_WEEK",		1 );
    defined('SPH_GROUPBY_MONTH') or define ( "SPH_GROUPBY_MONTH",		2 );
    defined('SPH_GROUPBY_YEAR') or define ( "SPH_GROUPBY_YEAR",		3 );
    defined('SPH_GROUPBY_ATTR') or define ( "SPH_GROUPBY_ATTR",		4 );
    defined('SPH_GROUPBY_ATTRPAIR') or define ( "SPH_GROUPBY_ATTRPAIR",	5 );
    class SphinxClient
    {
        var $_host = 'localhost';			///< searchd host (default is "localhost")
        var $_port = 9312;			///< searchd port (default is 9312)
        var $_offset = 0;		///< how many records to seek from result-set start (default is 0)
        var $_limit = 20;		///< how many records to return from result-set starting at offset (default is 20)
        var $_mode = SPH_MATCH_EXTENDED2;			///< query matching mode (default is SPH_MATCH_EXTENDED2)
        var $_weights = 1;		///< per-field weights (default is 1 for all fields)
        var $_sort = SPH_SORT_RELEVANCE;			///< match sorting mode (default is SPH_SORT_RELEVANCE)
        ...
        var $_warning = null;		///< last warning message
         ...
         * @return $this
         */
        function SetServer ( $host = '127.0.0.1', $port = 0 ){
