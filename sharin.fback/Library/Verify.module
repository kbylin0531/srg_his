...
class Verify {
    ...
    public function entry($id = '') {
        ...
        // 图片高(px)
        self::$config['imageH'] || self::$config['imageH'] = self::$config['fontSize'] * 2.5;
        // 建立一幅 self::$config['imageW'] x self::$config['imageH'] 的图像
        ...
        if(!function_exists('imagettftext')){
            SharinException::throwing('未安装GD库或者freetype库！');
        }
        if(self::$config['useZh']){ // 中文验证码
            for ($i = 0; $i<self::$config['length']; $i++) {
                ...
                imagettftext(
                    ...
                    mt_rand(-40, 40),
                    self::$config['fontSize']*($i+1)*1.5,
                    self::$config['fontSize'] + mt_rand(10, 20),
                    self::$_color,
                    self::$config['fontttf'],
                    $code[$i]);
            }
        ...
        // 保存验证码
        $key        =   $this->authcode(self::$config['seKey']);
    ...
    private function _writeCurve() {
        ...
        $T = mt_rand(self::$config['imageH'], self::$config['imageW']*2);  // 周期
        $w = (2* M_PI)/$T;
