...
class ThinkUploader extends Lite{
    ...
    const CONF_CONVENTION = [
        ...
        'rootPath'      =>  SR_PATH_BASE.'/Public/upload/', //保存根路径
        'savePath'      =>  '/', //保存路径
...
//        \Sharin\dumpout($finfo,$files);
        foreach ($this->dealFiles($files) as $key => $file) {
            $file['name']  = strip_tags($file['name']);
//            \Sharin\dumpout($savepath);
        ...
        if(!empty($info['download'])){
            if(strpos($savepath,SR_PATH_PUBLIC) === 0){
                $info['download']['savepath'] = substr($savepath,strlen(SR_PATH_PUBLIC)-1);
            }
            // the url whose cound be access
            $info['download']['access_url'] = SR_PUBLIC_URL.$info['download']['savepath'].$info['download']['savename'];
        }
