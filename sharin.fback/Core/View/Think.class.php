...
//        'DENY_FUNC_LIST'   =>  'echo,exit',    // 模板引擎禁用函数
    ...
    protected $_replacement = [
        '__PUBLIC__'    =>  SR_PUBLIC_URL,     // Public目录访问地址
    ];
    ...
    public function display($context = null, $cache_id = null, $compile_id = null, $parent = null) {
        ...
        $this->_context = $context;
        //模板常量
        defined('__ROOT__') or define('__ROOT__',SR_PUBLIC_URL);
        defined('__MODULE__') or define('__MODULE__',SR_PUBLIC_URL.'/'.$context['m']);
        ...
        $template or SharinException::throwing('No template!');
...
//        $tmplCacheFile = $folder.$prefix.md5($templateFile).$this->config['CACHFILE_SUFFIX'];
    ...
    protected function compiler($tmplContent) {
        ...
        // 添加安全代码
        $tmplContent =  '<?php if (!defined(\'SR_APP_NAME\')) exit(\'No Permission!\');?>'.$tmplContent;
        // 优化生成的php代码
    ...
    protected function parsePhp($content) {
        ...
        if($this->config['DENY_PHP'] && false !== strpos($content,'<?php')) {
            SharinException::throwing('denyed using raw php in template!');
        }
        return $content;
    }
...
//        $content    =   $this->parseLayout($content);
    ...
    private function parseXmlAttrs($attrs) {
        ...
        if(!$xml)  return SharinException::throwing('_XML_TAG_ERROR_:'.$attrs);
...
//                    }
    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            ...
            if(strpos($templateName,'/') !== 0){
                ...
                //Absolute path
                $templateName = SR_PATH_APP.$templateName;//abs
            }
            if(is_file($templateName)){
                // 获取模板文件内容
                $parseStr .= file_get_contents($templateName);
            }else{
                SharinException::throwing($templateName,'not found!');
            }
        }
        return $parseStr;
    }
    ...
    public function parseXmlAttr($attr,$tag) {
        ...
        $xml    =   simplexml_load_string($xml);
        $xml    or  SharinException::throwing('_XML_TAG_ERROR_:'.$attr);
        $xml    =   (array)($xml->tag->attributes());
            ...
            if($array) {
                ...
                foreach($attrs as $name) {
                    if( isset($array[$name])) {
                        ...
                        return SharinException::throwing('_PARAM_ERROR_:'.$name);
                    }
                }
                return $array;
            }
...
//            $basepath   =   !empty($tag['basepath'])?$tag['basepath']:SR_PATH_RUNTIME.'/Public';
            $basepath   =   !empty($tag['basepath'])?$tag['basepath']:SR_PATH_RUNTIME.'/Public';
            // 命名空间方式导入外部文件
            $array      =   explode(',',$file);
            foreach ($array as $val){
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="SR_PATH_RUNTIME/Js/Base.js" />
    public function _load($tag) {
        return $this->_import($tag,true);
    }

    // import别名使用 导入css文件 <css file="SR_PATH_RUNTIME/Css/Base.css" />
    public function _css($tag) {
        return $this->_import($tag,true,'css');
    }

    // import别名使用 导入js文件 <js file="SR_PATH_RUNTIME/Js/Base.js" />
...
//----------------------------------------- EXT -----------------------------------------------------------------------//
    ...
    public function _articles($attr,$content){
        ...
        $parseStr   =  "<?php 
                            isset(\$_dao) or \$_dao = \\Sharin\\Core\\Dao::getInstance();
                            \$params = {$params};
                            \$sql = '{$sql}';
                            \$list = \$_dao->query(\$sql,\$params);
                            if(false ===\$list){
                                \\Sharin\\Library\\Logger::write(\"failed to get article with sql:{$sql}\");
                                echo \"{$error}\";
                            }elseif(empty(\$list)){
                                echo \"{$empty}\";
                            }else{
