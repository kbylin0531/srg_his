...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public function readDir($dirpath, $recursion=false){
        if($this->checkReadableWithRevise($dirpath)){
            return $this->_readdir($dirpath,$recursion);
        }
        return [];
    }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public function touch($filepath, $mtime = null, $atime = null){
        ...
        $this->checkAndMakeSubdir($filepath) or SharinException::throwing("Check path '$filepath' failed");
